...
local function vector_components(vec)
    ...
    if (not x or not y or not z) and vec.GetX then
        local ok
        ok, x = pcall(vec.GetX, vec)
        if not ok then
            x = nil
        end
        ok, y = pcall(vec.GetY, vec)
        if not ok then
            y = nil
        end
        ok, z = pcall(vec.GetZ, vec)
        if not ok then
            z = nil
        end
    end
...
local function vector_distance(a, b)
    local ax, ay = vector_components(a)
    local bx, by = vector_components(b)
    local dx, dy = ax - bx, ay - by
    return math.sqrt((dx * dx) + (dy * dy))
end

local function get_enemy_detection_range()
    return tonumber(ui.enemy_range and ui.enemy_range:Get()) or 0
end

local function collect_enemies(hero, radius)
    local search_radius = tonumber(radius) or 0
    ...
    local function consider(enemy)
        if not enemy or enemy == hero or seen[enemy] then
            return
        end
        if not Entity.IsAlive(enemy) or Entity.IsDormant(enemy) or NPC.IsIllusion(enemy) then
            return
        end
        ...
        if search_radius > 0 then
            ...
            if vector_distance(hero_pos, enemy_pos) > search_radius then
                return
            end
        end
        seen[enemy] = true
    ...
    if NPC.GetHeroesInRadius then
        local enemies = NPC.GetHeroesInRadius(hero, search_radius, Enum.TeamType.TEAM_ENEMY)
        if enemies then
    ...
    if #result == 0 and Heroes and Heroes.Count then
...
local function find_enemy_for_ability(hero, ability, def)
    ...
    local search_radius = math.max(cast_range, get_enemy_detection_range())
    local enemies = collect_enemies(hero, search_radius)
...
local function compute_escape_position(hero, enemy, distance)
    local hero_pos = Entity.GetAbsOrigin(hero)
    local hx, hy, hz = vector_components(hero_pos)

    local dx, dy
    if enemy then
        local enemy_pos = Entity.GetAbsOrigin(enemy)
        local ex, ey = vector_components(enemy_pos)
        dx, dy = hx - ex, hy - ey
    else
        local forward = Entity.GetForwardPosition(hero, 10)
        local fx, fy = vector_components(forward)
        dx, dy = hx - fx, hy - fy
    end
    ...
    local target_x = hx + (nx * distance)
    local target_y = hy + (ny * distance)

    return Vector(target_x, target_y, hz)
end
...
local function queue_blink_after_eul(eul_def, hero, detection_enemies, blink_def, blink_ability)
    ...
    local enemy = find_closest_enemy(hero, math.max(get_enemy_detection_range(), blink_distance))
    if not enemy then
        return false
    end
...
local function process_pending_eul_blink(hero, detection_enemies)
    ...
    if not enemy or not Entity.IsAlive(enemy) or Entity.IsDormant(enemy) or NPC.IsIllusion(enemy) then
        enemy = find_closest_enemy(hero, math.max(get_enemy_detection_range(), entry.distance or blink_def.blink_range or 1200))
    end
...
local function cast_item(def, hero, detection_enemies)
    ...
    elseif def.cast == "blink_escape" then
        ...
        local search_radius = math.max(get_enemy_detection_range(), def.blink_range or 1200)
    ...
    elseif def.cast == "force_escape" then
        local search_radius = math.max(get_enemy_detection_range(), def.escape_distance or 600)
        local enemy = find_closest_enemy(hero, search_radius)
        if not enemy then
            return false
        end
...
function auto_defender.OnUpdate()
    ...
    local current_health = health_percent(hero)
    local detection_enemies = collect_enemies(hero, get_enemy_detection_range())
